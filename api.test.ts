...
import { addresses, applicationAnswers, protocols } from './constants';
...
describe('Protocol tests', () => {
    ...
    for (const protocol of protocols) {
        it('Should create a Protocol', async () => {
            ...
            const req = request(app)
                .post('/api/protocol/createProtocol')
                .set('Authorization', `Bearer ${authorization.token}`);

            for (const [name, value] of serialize(protocol.original, { indices: true })) {
                req.field(name, value.toString());
            }
            ...
            it('Should update a Protocol', async () => {
                ...
                const req = request(app)
                    .put(`/api/protocol/updateProtocol/${protocol.original.id}`)
                    .set('Authorization', `Bearer ${authorization.token}`);
    
                for (const [name, value] of serialize(protocol.updated, { indices: true })) {
                    req.field(name, value.toString());
                }
        ...
        it('Should get all ProtocolAnswers', async () => {
            const expectedResponse = {
            ...
            const response = await request(app)
                .get('/api/protocol/getAllProtocol')
                .set('Authorization', `Bearer ${authorization.token}`);
        ...
        it('Should get a Protocol by id', async () => {
            const expectedResponse = {
            ...
            const response = await request(app)
                .get(`/api/protocol/getProtocol/${protocol.original.id}`)
                .set('Authorization', `Bearer ${authorization.token}`);
        ...
        it('Should delete a Protocol', async () => {
            ...
            const response = await request(app)
                .delete(`/api/protocol/deletProtocol/${protocol.original.id}`)
                .set('Authorization', `Bearer ${authorization.token}`);
